...
function Step1({ data, set }) {
  ...
  const showGround = loanProduct === "Ground Up Construction";
  const showDSCR = loanProduct === "DSCR Rental Loan";
  const isReverseMortgage = loanProduct === "Reverse Mortgage";
  ...
  return (
    <div>
      ...
      {isReverseMortgage && (
        <Row>
          <Select
            label="What's the estimated property value?"
            required
            options={[
              "Less than $75,000",
              "$75,000 - $100,000",
            ...
            onChange={(e) => set("estimatedPropertyValue", e.target.value)}
          />
          <Select
            label="What's your mortgage balance?"
            required
            options={[
              "$0 - $50,000",
              "$50,001 - $100,000",
      ...
      {isReverseMortgage && (
        <Row cols={1}>
          <Input
            label="What is the youngest borrower's age?"
            required
            type="number"
            placeholder="e.g. 62"
            value={data.youngestBorrowerAge || ""}
            onChange={(e) => set("youngestBorrowerAge", e.target.value)}
          />
        </Row>
      )}

      {/* ── Purchase / Refi (hidden for Reverse Mortgage) ── */}
      {!isReverseMortgage && (
        <>
          <RadioGroup
            label="Is this a Purchase or Refinance?"
...
export default function KreativeLending() {
  ...
  const submitToGHL = async (data) => {
    const payload = {
      ...
      youngestBorrowerAge: data.youngestBorrowerAge || "",
      // Step 2
      propertyAddress: data.propertyAddress || "",
      propertyType: data.propertyType || "",
      propertyTypeOther: data.propertyTypeOther || "",
